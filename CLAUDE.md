...
| `XPC_RESPAWN_S`    | 90 s    | Services reappearing within this window are blocklisted (must exceed `IDLE_SWEEP_S`=30s or sweep-killed services evade detection — see Change Log 2.6.1) |
...
| Menu bar requires `rumps` | `_start_menubar()` silently no-ops if `rumps` is not installed. Install with `pip install rumps`. **Must own the main thread** (fixed in 2.6.1 — see Change Log; previously launched on a background thread and crashed immediately on every start with `NSInternalInconsistencyException`, so the menu bar never actually appeared). `main()` now runs `_start_menubar()` on the main thread and the HTTP server in a background thread instead. |
...
|------------|---------|----------------|------------------------------------------------------------------------|
| 2026-08-01 | 2.6.1   | itsmeSugunakar | **Fix: XPC respawn guard evaded by idle-sweep kill loops** — `XPC_RESPAWN_S` (10s) was shorter than `IDLE_SWEEP_S` (30s), so any service killed by `_sweep_idle_services()` and relaunched by launchd before the *next* sweep evaded `_detect_xpc_respawn()` entirely — it never crossed the 10s-reappearance check, so it never got blocklisted and was killed and relaunched indefinitely. Caught live: `WallpaperVideoExtension` (macOS video-wallpaper renderer) respawning every 30–64s, spiking to ~97% CPU each launch, and visibly flickering the desktop — the same undetected loop was simultaneously hitting 8 other launchd services (`VTDecoderXPCService`, `AppPredictionIntentsHelperService`, `SiriSuggestionsBookkeepingService`, `MTLCompilerService`, `SiriNCService`, `SAExtensionOrchestrator`, `IntelligencePlatformComputeService`, `CloudTelemetryService`). Raised `XPC_RESPAWN_S` to 90s so it reliably exceeds the sweep cadence; on restart the guard immediately caught and blocklisted all 9 services in one pass. **Fix: menu bar never actually rendered** — `_start_menubar()` (rumps/AppKit) was launched via `threading.Thread(..., daemon=True)` in `main()`, but `NSStatusBar`/`NSStatusItem` creation raises `NSInternalInconsistencyException: NSWindow should only be instantiated on the main thread!` when not called from the main thread — it crashed on every single startup, silently, with the icon never appearing (present since the menu bar feature was introduced in 2.1.0). `main()` now runs the `ThreadingHTTPServer` in a background thread and reserves the main thread for `_start_menubar()`; falls back to `server_thread.join()` when `rumps` isn't installed. `_start_menubar()` now returns `bool` (whether it actually ran) instead of `None`. Also includes a pending Summary tab update: **Top Resource Consumers** panel replaces "Top Memory Eaters" with a Memory/CPU sort toggle, per-row LEAK/THROTTLED badges, and inline Freeze/Throttle action buttons. |
